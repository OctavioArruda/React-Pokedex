...
export default class Pokemon extends Component {
    ...
    async componentDidMount() {
        ...
        // decimeters to centimers: divide by 10
        const height = Math.round(pokemonResponse.data.height / 10).toFixed(2);
        // hectograms to grams: multiply by 1000
        const weight = Math.round(pokemonResponse.data.weight * 100).toFixed(2);
        const types = pokemonResponse.data.types.map(type => type.type.name);
        const abilities = pokemonResponse.data.abilities.map(ability => ability.ability.name);
        const evs = pokemonResponse.data.stats.filter(stat => stat.effort > 0).map(stat => `${stat.effort} ${stat.stat.name}`);
